...
const imgPath = process.env.NODE_ENV === 'development' ? '' : 'dist/';
const favIconLevel = process.env.NODE_ENV === 'development' ? false : true;
const root = `${__dirname}/src`;

// // const isPage = new RegExp(/(\/page-)(.*)(\.html)(.*)/);
// const isIndex = new RegExp(/(\/index\.html)(.*)/);

const eachURL = (url, attr, element) => {
	if (process.env.NODE_ENV === 'production') {
		console.log(url);

		// // if (isPage.test(url)) { url = url.replace('.html', '/').replace('/page-', '/'); }
		// if (isIndex.test(url)) { url = url.replace('.html', '/').replace('/index', '/'); }
	}
	return url;
};
...
module.exports = {
	plugins: [
		...
		// require('posthtml-urls')({ eachURL }), // https://github.com/posthtml/posthtml-urls
	]
}